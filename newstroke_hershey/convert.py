...
# - NewStroke project:         http://vovanium.ru/sledy/newstroke/en
...
# Licence text: http://creativecommons.org/publicdomain/zero/1.0/
...
def format_number(num):
    """
    Properly format a floating point number according to the fontobene specifications.
    """
...
if __name__ == '__main__':
    ...
    ret = os.system('awk -f fontconv.awk symbol.lib font.lib charlist.txt > newstroke_font.c')
    assert(ret == 0)

    # generate and load shared library
    ret = os.system('gcc -shared -o newstroke_font.so newstroke_font.c')
    assert (ret == 0)
    ...
    for i, glyph in enumerate(glyphs):
        codepoint = i + 0x20
        if glyph == REPLACEMENT_GLYPH:  # remove all replacement glyphs as they are useless
            continue
